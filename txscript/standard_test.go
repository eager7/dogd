...
// newAddressPubKey returns a new dogutil.AddressPubKey from the provided
...
func newAddressPubKey(serializedPubKey []byte) dogutil.Address {
	addr, err := dogutil.NewAddressPubKey(serializedPubKey,
...
// newAddressPubKeyHash returns a new dogutil.AddressPubKeyHash from the
// provided hash.  It panics if an error occurs.  This is only used in the tests
// as a helper since the only way it can fail is if there is an error in the
// test source code.
func newAddressPubKeyHash(pkHash []byte) dogutil.Address {
	addr, err := dogutil.NewAddressPubKeyHash(pkHash, &chaincfg.MainNetParams)
	if err != nil {
...
// newAddressScriptHash returns a new dogutil.AddressScriptHash from the
// provided hash.  It panics if an error occurs.  This is only used in the tests
// as a helper since the only way it can fail is if there is an error in the
// test source code.
func newAddressScriptHash(scriptHash []byte) dogutil.Address {
	addr, err := dogutil.NewAddressScriptHashFromHash(scriptHash,
		&chaincfg.MainNetParams)
	if err != nil {
...
// bogusAddress implements the dogutil.Address interface so the tests can ensure
...
// IsForNet lies blatantly to satisfy the dogutil.Address interface.
...
func TestPayToAddrScript(t *testing.T) {
	...
	p2pkhMain, err := dogutil.NewAddressPubKeyHash(hexToBytes("e34cce70c86"+
	...
	p2shMain, _ := dogutil.NewAddressScriptHashFromHash(hexToBytes("e8c300"+
	...
	p2pkCompressedMain, err := dogutil.NewAddressPubKey(hexToBytes("02192d"+
	...
	p2pkCompressed2Main, err := dogutil.NewAddressPubKey(hexToBytes("03b0b"+
	...
	p2pkUncompressedMain, err := dogutil.NewAddressPubKey(hexToBytes("0411"+
	...
	tests := []struct {
		...
		{(*dogutil.AddressPubKeyHash)(nil), "", errUnsupportedAddress},
		{(*dogutil.AddressScriptHash)(nil), "", errUnsupportedAddress},
		{(*dogutil.AddressPubKey)(nil), "", errUnsupportedAddress},
...
func TestMultiSigScript(t *testing.T) {
	...
	tests := []struct {
		keys      []*dogutil.AddressPubKey
		nrequired int
		expected  string
		err       error
	}{
		{
			[]*dogutil.AddressPubKey{
				p2pkCompressedMain,
				p2pkCompressed2Main,
			},
			1,
