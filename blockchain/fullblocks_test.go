...
import (
	...
	"github.com/eager7/dogd/blockchain/fullblocktests"
...
func TestFullBlocks(t *testing.T) {
	...
	testRejectedNonCanonicalBlock := func(item fullblocktests.RejectedNonCanonicalBlock) {
		...
		err := msgBlock.BchDecode(bytes.NewReader(item.RawBlock), 0, wire.BaseEncoding)
...
// TestCTORActivation ensures all tests generated by the fullblocktests package
// have the expected result when processed via ProcessBlock.
func TestCTORActivation(t *testing.T) {
	tests, err := fullblocktests.GenerateWithTxs(10, 10)
	if err != nil {
		t.Fatalf("failed to generate tests: %v", err)
	}
...
func TestGreatWallActivation(t *testing.T) {
	tests, err := fullblocktests.GenerateGreatWallTestBlocks()
	if err != nil {
		t.Fatalf("failed to generate tests: %v", err)
	}
...
// TestGravitonActivation tests that Schnorr multi-signatures validate properly
// and the MinmalData rule activates during Graviton activation.
func TestGravitonActivation(t *testing.T) {
	tests, err := fullblocktests.GenerateGravitonTestBlocks()
	if err != nil {
		t.Fatalf("failed to generate tests: %v", err)
	}
