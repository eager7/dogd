...
var (
	...
	bchctlHomeDir         = bchutil.AppDataDir("bchctl", false)
	bchwalletHomeDir      = bchutil.AppDataDir("bchwallet", false)
	defaultConfigFile     = filepath.Join(bchctlHomeDir, "bchctl.conf")
	defaultRPCServer      = "localhost"
	defaultRPCCertFile    = filepath.Join(bchdHomeDir, "rpc.cert")
	defaultWalletCertFile = filepath.Join(bchwalletHomeDir, "rpc.cert")
)
...
func cleanAndExpandPath(path string) string {
	...
	if strings.HasPrefix(path, "~") {
		homeDir := filepath.Dir(bchctlHomeDir)
...
func loadConfig() (*config, []string, error) {
	...
	if _, err := os.Stat(preCfg.ConfigFile); os.IsNotExist(err) {
		// Use config file for RPC server to create default bchctl config
		var serverConfigPath string
		if preCfg.Wallet {
			serverConfigPath = filepath.Join(bchwalletHomeDir, "bchwallet.conf")
		} else {
			serverConfigPath = filepath.Join(bchdHomeDir, "bchd.conf")
		}
