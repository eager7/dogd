...
primitives necessary to speak the bitcoin cash wire protocol, in order to
...
A quick overview of the major features peer provides are as follows:
 ...
 - Provides a basic concurrent safe bitcoin cash peer for handling bitcoin
...
caller to specify things such as the user agent name and version, the bitcoin cash
network to use, which services it supports, and callbacks to invoke when bitcoin
cash messages are received.  See the documentation for each field of the Config
...
cash messages.  This is accomplished by creating an instance of the MessageListeners
...
For convenience, a callback hook for all of the currently supported bitcoin cash
...
(https://godoc.org/github.com/eager7/dogd/wire#hdr-Bitcoin_Improvement_Proposals)
*/
package peer
