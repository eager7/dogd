...
func (s *fakeChain) FetchUtxoView(tx *dogutil.Tx) (*blockchain.UtxoViewpoint, error) {
	s.RLock()
	defer s.RUnlock()
...
func (s *fakeChain) CalcSequenceLock(tx *dogutil.Tx,
...
// transactions.
func txOutToSpendableOut(tx *dogutil.Tx, outputNum uint32) spendableOutput {
	return spendableOutput{
		outPoint: wire.OutPoint{Hash: *tx.Hash(), Index: outputNum},
		amount:   dogutil.Amount(tx.MsgTx().TxOut[outputNum].Value),
	}
}
...
// version 2 blocks.
func (p *poolHarness) CreateCoinbaseTx(blockHeight int32, numOutputs uint32) (*dogutil.Tx, error) {
...
func (p *poolHarness) CreateSignedTx(inputs []spendableOutput,
	numOutputs uint32, fee dogutil.Amount,
	signalsReplacement bool) (*dogutil.Tx, error) {

	// Calculate the total input amount and split it amongst the requested
	// number of outputs.
	var totalInput dogutil.Amount
...
func (p *poolHarness) CreateTxChain(firstOutput spendableOutput, numTxns uint32) ([]*dogutil.Tx, error) {
	txChain := make([]*dogutil.Tx, 0, numTxns)
	...
	for i := uint32(0); i < numTxns; i++ {
		...
		txChain = append(txChain, dogutil.NewTx(tx))
...
func newPoolHarness(chainParams *chaincfg.Params) (*poolHarness, []spendableOutput, error) {
	...
	payPubKeyAddr, err := dogutil.NewAddressPubKey(pubKeyBytes, chainParams)
	if err != nil {
		return nil, nil, err
	}
...
// mock chain.
func (ctx *testContext) addCoinbaseTx(numOutputs uint32) *dogutil.Tx {
	ctx.t.Helper()
...
func (ctx *testContext) addSignedTx(inputs []spendableOutput,
	numOutputs uint32, fee dogutil.Amount,
	signalsReplacement, confirmed bool) *dogutil.Tx {
...
func testPoolMembership(tc *testContext, tx *dogutil.Tx, inOrphanPool, inTxPool bool) {
	tc.t.Helper()
...
func TestBasicOrphanRemoval(t *testing.T) {
	...
	nonChainedOrphanTx, err := harness.CreateSignedTx([]spendableOutput{{
		amount:   dogutil.Amount(5000000000),
...
func TestSignalsReplacement(t *testing.T) {
	...
	testCases := []struct {
		name               string
		setup              func(ctx *testContext) *dogutil.Tx
...
func TestConflicts(t *testing.T) {
	...
	testCases := []struct {
		...
		setup func(ctx *testContext) (*dogutil.Tx, []*dogutil.Tx)
	}{
		{
		...
		},
		{
			...
			setup: func(ctx *testContext) (*dogutil.Tx, []*dogutil.Tx) {
				...
				return tx, []*dogutil.Tx{conflict1, conflict2}
			},
		},
		{
			// Create a transaction that would introduce two
			...
			setup: func(ctx *testContext) (*dogutil.Tx, []*dogutil.Tx) {
				...
				return tx, []*dogutil.Tx{parent, child}
			},
		},
	}
...
func TestRBF(t *testing.T) {
	...
	const defaultFee = dogutil.SatoshiPerBitcoin
